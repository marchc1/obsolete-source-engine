...
struct lua_State
{
    ...
    ILuaInterface* luabase;
        ...
        #define GMOD_MODULE_OPEN()                                  \
            int gmod13_open__Imp( ILuaInterface* LUA );  \
            GMOD_DLL_EXPORT int gmod13_open( lua_State* L )         \
            {                                                       \
                return gmod13_open__Imp( L->luabase );              \
            }                                                       \
            int gmod13_open__Imp( [[maybe_unused]] ILuaInterface* LUA )

        #define GMOD_MODULE_CLOSE()                                 \
            int gmod13_close__Imp( ILuaInterface* LUA ); \
            GMOD_DLL_EXPORT int gmod13_close( lua_State* L )        \
            {                                                       \
                return gmod13_close__Imp( L->luabase );             \
            }                                                       \
            int gmod13_close__Imp( [[maybe_unused]] ILuaInterface* LUA )

        #define LUA_FUNCTION( FUNC )                            \
            int FUNC##__Imp( ILuaInterface* LUA );   \
            int FUNC( lua_State* L )                            \
            {                                                   \
                ILuaInterface* LUA = L->luabase;     \
                LUA->SetState(L);                               \
                return FUNC##__Imp( LUA );                      \
            }                                                   \
            int FUNC##__Imp( [[maybe_unused]] ILuaInterface* LUA )
        ...
        #define LUA_FUNCTION_STATIC( FUNC )                             \
            ...
            static int FUNC##__Imp( [[maybe_unused]] ILuaInterface* LUA )
